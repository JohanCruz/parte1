...
router.get('/resumen/:date',(req, res) =>{
    ...
    while(rangeOfTime < parseInt(req.query.dias) || parseInt(req.query.dias) === 0 ){
        ...
        const slideDate = parseInt(initialDate.getDate()) < 10 ? "0"+ initialDate.getDate().toString() : initialDate.getDate().toString()
        ...
        const responsew = await fetch(url, settings);
                
        let clients = await responsew.json();                
        ...
        console.log("rangeOfTime", rangeOfTime);        
